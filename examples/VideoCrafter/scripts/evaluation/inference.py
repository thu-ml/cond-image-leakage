...
def get_parser():
    ...
    parser.add_argument("--M", type=int, default=1000, help="timestep that inference starts")
    parser.add_argument("--analytic_init_path", type=str, help="path to the analytic-init vector",default="")
...
def run_inference(args, gpu_num, gpu_no, **kwargs):
    ...
    frames = 16 #model.temporal_length if args.frames < 0 else args.frames
    ...
    for idx in range(0, n_rounds):
        ...
        sqrt_alpha_t=model.get_sqrt_alpha_t_bar(expectation_X_0,torch.tensor([args.M-1]).to('cuda'))
        ...
        batch_samples = batch_ddim_sampling(model, cond, noise_shape, args.n_samples, \
                                                args.ddim_steps, args.ddim_eta, args.unconditional_guidance_scale,ddpm_from=args.M,x_T=init, **kwargs)
        ## b,samples,c,t,h,w
