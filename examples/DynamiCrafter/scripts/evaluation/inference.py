...
def image_guided_synthesis(model, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
                        unconditional_guidance_scale=1.0, cfg_img=None, fs=None, text_input=False, multiple_cond_cfg=False, loop=False, interp=False, timestep_spacing='uniform_trailing', guidance_rescale=0.0,M=1000,**kwargs):
    ...
    for _ in range(n_samples):
        ...
        batch_x_0_pred=model.decode_first_stage(x_0_preds[1])
        batch_variants_x_0_preds.append(batch_x_0_pred)
    ## variants, batch, c, t, h, w
    batch_variants = torch.stack(batch_variants)
    batch_variants_x_0_preds=torch.stack(batch_variants_x_0_preds)
    return batch_variants.permute(1, 0, 2, 3, 4, 5),batch_variants_x_0_preds.permute(1, 0, 2, 3, 4, 5)

def image_guided_synthesis_analytic(model, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
                        unconditional_guidance_scale=1.0, cfg_img=None, fs=None, text_input=False, multiple_cond_cfg=False, loop=False, interp=False, timestep_spacing='uniform_trailing', guidance_rescale=0.0,M=1000,analytic_init_path=None,**kwargs):
...
def run_inference(args, gpu_num, gpu_no):
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            if args.whether_analytic_init==0:
                ...
                batch_samples = image_guided_synthesis_analytic(model, prompts, videos, noise_shape, args.n_samples, args.ddim_steps, args.ddim_eta, \
                            args.unconditional_guidance_scale, args.cfg_img, args.frame_stride, args.text_input, args.multiple_cond_cfg, args.loop, args.interp, args.timestep_spacing, args.guidance_rescale
                            ,M=args.M,analytic_init_path=args.analytic_init_path)
