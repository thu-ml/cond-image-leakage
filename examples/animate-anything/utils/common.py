...
def DDPM_forward_timesteps(x0, step, num_frames, scheduler):
    ...
    alpha_T=torch.tensor([[[[[0.0903]]]]], device=device, dtype=torch.float16)
    sigma_T=torch.tensor([[[[[0.9956]]]]], device=device, dtype=torch.float16)
    xt=alpha_T * xt + sigma_T * noise
    #xt = scheduler.add_noise(xt, noise, t)
    #print(f'\n\n---------\nsqrt_alpha_{timesteps[0]} is {scheduler.get_sqrt_alpha_t_bar(xt, noise, timesteps[0])}\n----------\n\n')
    return xt, timesteps

def DDPM_forward_timesteps_analytic_init(x0, step, num_frames, scheduler,mean,std):
    '''larger step -> smaller t -> smaller alphas[t:] -> smaller xt -> smaller x0'''
    ...
    #print(f'\n\n---------\nsqrt_alpha_{timesteps[0]} is {scheduler.get_sqrt_alpha_t_bar(xt, noise, timesteps[0])}\n----------\n\n')
    mean.to(device)
    std=torch.tensor([[[[[std]]]]], device=device, dtype=torch.float16)
    xt=mean+std*noise
    return xt, timesteps
